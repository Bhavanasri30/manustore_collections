...
export default function CustomerAuthPage() {
  const { registerCustomer, loginCustomer, resetPassword, setError, error } = useStore();
  ...
  return (
    <div className="flex min-h-[calc(100vh-120px)] items-center justify-center px-4 py-10">
      <div className="grid w-full max-w-5xl overflow-hidden rounded-[32px] border border-[#eadbc7] bg-white shadow-[0_20px_50px_rgba(91,31,45,0.12)] lg:grid-cols-[1fr,1.15fr]">
        <div className="relative hidden bg-[radial-gradient(circle_at_top,_#f8ebd8,_#f4e4d2_35%,_#edd6bb_100%)] p-8 text-[#38131d] lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative z-10">
            ...
            <h2 className="max-w-xs text-3xl font-bold leading-tight">
              Discover handcrafted fashion made for everyday moments.
            </h2>
          </div>

          <div className="relative z-10 space-y-4 rounded-[26px] border border-white/40 bg-white/35 p-5 backdrop-blur-sm">
            <div className="flex items-center justify-between text-sm font-medium text-[#5b1f2d]">
        ...
        <div className="p-6 sm:p-8 lg:p-10">
          {!resetMode ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {mode === 'signup' && (
                  <div>
                    <label className="mb-2 block text-sm font-medium text-[#463535]">Full name</label>
                    <input name="name" value={form.name} onChange={handleChange} className="w-full rounded-2xl border border-[#eadbc7] bg-[#fffdfb] px-3.5 py-3 text-sm text-[#38131d] outline-none transition focus:border-[#5b1f2d] focus:ring-2 focus:ring-[#eadbc7]" placeholder="Enter your name" />
                  </div>
                )}
                <div>
                  <label className="mb-2 block text-sm font-medium text-[#463535]">Email</label>
                  <input type="email" name="email" value={form.email} onChange={handleChange} className="w-full rounded-2xl border border-[#eadbc7] bg-[#fffdfb] px-3.5 py-3 text-sm text-[#38131d] outline-none transition focus:border-[#5b1f2d] focus:ring-2 focus:ring-[#eadbc7]" placeholder="you@example.com" />
                </div>
                <div>
              ...
              {mode === 'login' && (
                <div className="mt-4 flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="text-[#7a6664]">Secure checkout</span>
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <form onSubmit={handleResetSubmit} className="space-y-4">
                <div>
                  <label className="mb-2 block text-sm font-medium text-[#463535]">Email</label>
                  <input type="email" name="email" value={resetForm.email} onChange={handleResetChange} className="w-full rounded-2xl border border-[#eadbc7] bg-[#fffdfb] px-3.5 py-3 text-sm text-[#38131d] outline-none transition focus:border-[#5b1f2d] focus:ring-2 focus:ring-[#eadbc7]" placeholder="you@example.com" />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium text-[#463535]">New password</label>
                  <input type="password" name="newPassword" value={resetForm.newPassword} onChange={handleResetChange} className="w-full rounded-2xl border border-[#eadbc7] bg-[#fffdfb] px-3.5 py-3 text-sm text-[#38131d] outline-none transition focus:border-[#5b1f2d] focus:ring-2 focus:ring-[#eadbc7]" placeholder="New password" />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium text-[#463535]">Confirm password</label>
                  <input type="password" name="confirmPassword" value={resetForm.confirmPassword} onChange={handleResetChange} className="w-full rounded-2xl border border-[#eadbc7] bg-[#fffdfb] px-3.5 py-3 text-sm text-[#38131d] outline-none transition focus:border-[#5b1f2d] focus:ring-2 focus:ring-[#eadbc7]" placeholder="Repeat your password" />
                </div>
          ...
          <div className="mt-6 border-t border-[#f0e3d7] pt-4 text-center text-sm text-[#5d4c4d]">
            <Link to="/owner-login" className="font-semibold text-[#5b1f2d] transition hover:text-[#3b1b1f]">
              Store owner login
            </Link>
          </div>
        </div>
